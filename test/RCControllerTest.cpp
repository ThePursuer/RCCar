...
TEST_F(RCControllerFixture, directionTest){
	ASSERT_EQ(FORWARD, test_car->getDirection());
	...
	test_controller->handleJoystickEvent(test_js_event);
	ASSERT_EQ(BACKWARD, test_car->getDirection());

	test_js_event.number = 2;
	test_js_event.value = INT16_MIN;//The xbox controller uses signed shorts for triggers so we will use them in this test as we would in the actuall code
