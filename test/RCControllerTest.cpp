...
TEST_F(RCControllerFixture, turnRightTest){
	...
	ASSERT_NEAR(2.0, test_car->getServoPW(), 0.1);
	...
	test_controller->handleJoystickEvent(test_js_event);
	ASSERT_NEAR(1.5, test_car->getServoPW(), 0.1);
	...
	test_controller->handleJoystickEvent(test_js_event);
	ASSERT_NEAR(1.0, test_car->getServoPW(), 0.1);
}
...
TEST_F(RCControllerFixture, goForwardTest){
	...
	test_controller->handleJoystickEvent(test_js_event);
	ASSERT_NEAR((UINT8_MAX / 2), test_car->getSpeed(), 1);
